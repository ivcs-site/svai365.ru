...
if ($_POST['name_form'] == 'request-reconstruct') {
    ...
    $mail->Body    = 'Имя: <b> ' . $name . '</b><br> Телефон: <b> ' . $phone . '</b><br> E-mail: <b> ' . $email;
    $mail->AltBody = '';
    $mail->send();
    ...
    $mail->AltBody = '';
    $mail->send();
} elseif ($_POST['name_form'] == 'request-consult') {
    $name = $_POST['name'];
    $phone = $_POST['phone'];
    $email = $_POST['email'];

    $mail->Subject = 'Получение консультации';
    $mail->Body    = 'Имя: <b> ' . $name . '</b><br> Телефон: <b> ' . $phone . '</b><br> E-mail: <b> ' . $email;
    $mail->AltBody = '';
    $mail->send();
    ...
    $mail->AltBody = '';
    $mail->send();
} elseif ($_POST['name_form'] == 'request-home') {
    $name = $_POST['name'];
    $phone = $_POST['phone'];
    $email = $_POST['email'];

    $mail->Subject = 'Заказ на дом';
    $mail->Body    = 'Имя: <b> ' . $name . '</b><br> Телефон: <b> ' . $phone . '</b><br> E-mail: <b> ' . $email;
    $mail->AltBody = '';
    $mail->send();
} elseif ($_POST['name_form'] == 'request-franchise') {
    $name = $_POST['name'];
    $phone = $_POST['phone'];
    $email = $_POST['email'];

    $mail->Subject = 'Купить франшизу';
    $mail->Body    = 'Имя: <b> ' . $name . '</b><br> Телефон: <b> ' . $phone . '</b><br> E-mail: <b> ' . $email;
    $mail->AltBody = '';
    $mail->send();
